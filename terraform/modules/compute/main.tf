...
resource "aws_security_group" "ec2" {
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["10.0.0.0/16"] # Bastion access only
  }
  ...
  tags = {
    Name = "starttech-${var.environment}-ec2-sg"
  }
...
resource "aws_iam_role" "ec2" {
  name = "starttech-${var.environment}-ec2-role"
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:logs:*:*:log-group:/starttech/*"
      },
      {
        Effect = "Allow"
        Action = [
...
resource "aws_iam_instance_profile" "ec2" {
  name = "starttech-${var.environment}-ec2-profile"
  role = aws_iam_role.ec2.name
}

# Launch Template
resource "aws_launch_template" "app" {
  name_prefix   = "starttech-${var.environment}-"
...
resource "aws_s3_bucket" "logs" {
  bucket = "starttech-${var.environment}-alb-logs-${data.aws_caller_identity.current.account_id}"

  tags = {
    Name = "starttech-alb-logs"
  }
}
...
resource "aws_autoscaling_policy" "cpu_scale_up" {
  name                   = "starttech-${var.environment}-cpu-scale-up"
...
resource "aws_autoscaling_policy" "cpu_scale_down" {
  name                   = "starttech-${var.environment}-cpu-scale-down"
  scaling_adjustment     = -1
  adjustment_type        = "ChangeInCapacity"
  cooldown               = 300
  autoscaling_group_name = aws_autoscaling_group.app.name
